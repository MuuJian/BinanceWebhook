...
提醒会按 3%、6%、9%、12%……逐级触发。例如一次快速跌到 10%，会依次提醒
3%、6%、9%，每 30 秒最多发送一次；发送前会重新确认当前波动仍满足对应档位。
价格回落时不会倒着重复提醒。只有波动回到 2%或以内并连续保持 10 秒，才会开始
新一轮；2%到3%是防止边界抖动的缓冲区。
...
```text
BTCUSDT 合约2分钟内上涨3.24%，触发3%档提醒，当前价格70120.5，参考价格67920
ETHUSDT 合约2分钟内下跌3.12%，触发3%档提醒，当前价格3686，参考价格3800
```
